...
void Bitboards::init()
{
  ...
  auto effectCalc = [](const Square square, const Bitboard& occupied, const Piece piece)
  {
    ...
    for (auto delta : deltaArray[(piece == BISHOP) ? 0 : 1])
      ...
      for (Square sq = square + delta; is_ok(sq) && dist(sq - delta, sq) <= 2; sq += delta)
      {
  ...
  for (auto s1 : SQ)
    for (auto s2 : SQ)
    {
      ...
      {
        // 間に挟まれた升を1に
        Square delta = (s2 - s1) / max(abs(file_of(s2) - file_of(s1)), abs(rank_of(s2) - rank_of(s1)));
        for (Square s = s1 + delta; s != s2; s += delta)
          BetweenBB[s1][s2] |= s;

        // 間に挟まれてない升も1に
        LineBB[s1][s2] = BetweenBB[s1][s2];
        for (Square s = s1; is_ok(s) && dist(s, s + delta) <= 2; s -= delta) // s1から-delta方向に延長
          LineBB[s1][s2] |= s;
        for (Square s = s2; is_ok(s) && dist(s, s - delta) <= 2; s += delta) // s2から+delta方向に延長
          LineBB[s1][s2] |= s;
      }
    }
